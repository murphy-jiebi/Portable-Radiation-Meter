...
void Display_doseVal(uint8_t x, uint8_t y,float val, uint8_t unit)
{
	...
	{
		case 1:
			GUI_SetString2412(x+70,y+60,p==0?"uSv/h":"mSv/h",0);
			break;
		case 2:
			GUI_SetString2412(x+70,y+60,p==0?"uGy/h":"mGy/h",0);
			break;
		case 0:
			GUI_SetString2412(x+70,y+60,p==0?"cps":"Kcps",0);
			break;
		default :break;
	}
...
void Display_CNT(uint32_t cnt1, uint32_t cnt2)
{
	char arry[20]={0};

	sprintf(arry,"%d",cnt1);
	GUI_SetString4824(50,24,arry,0);

	sprintf(arry,"%d",cnt2);
	GUI_SetString4824(50,80,arry,0);
}


void Display_setFactor(uint8_t x, uint8_t y, uint8_t val[3], uint8_t index)
{
	uint8_t width = 0;
	
	GUI_SetChar4824(x+24*2,y,val[0],index==0?1:0);
	GUI_SetChar2412(x+72,y+16,'.',0);
	GUI_SetChar4824(x+84,y,val[1],index==1?1:0);
	GUI_SetChar4824(x+108,y,val[2],index==2?1:0);
	
}

void Display_setDeadtime(uint8_t x, uint8_t y, uint8_t val[8], uint8_t index)
{
	uint8_t width = 0;
	
	GUI_SetChar4824(x+24*0,y,val[0],index==0?1:0);
	GUI_SetChar2412(x+24,y+16,'.',0);
	GUI_SetChar4824(x+24*0+36,y,val[1],index==1?1:0);
	GUI_SetChar4824(x+24*1+36,y,val[2],index==2?1:0);
	GUI_SetChar4824(x+24*2+36,y,val[3],index==3?1:0);
	GUI_SetChar4824(x+24*3+36,y,val[4],index==4?1:0);
	GUI_SetChar4824(x+24*4+36,y,val[5],index==5?1:0);
	GUI_SetChar4824(x+24*5+36,y,val[6],index==6?1:0);
	GUI_SetChar4824(x+24*6+36,y,val[7],index==7?1:0);

}

void Display_setDtFactor(uint8_t x, uint8_t y, uint8_t val[5], uint8_t index)
{
	uint8_t width = 0;
