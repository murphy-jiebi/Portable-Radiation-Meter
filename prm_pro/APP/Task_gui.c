...
void menu611(uint8_t key);
void menu61(uint8_t key);
void menu62(uint8_t key);
void menu63(uint8_t key);
void menu621(uint8_t key);
void menu631(uint8_t key);
void menu231(uint8_t key);
...
/*index	 1  2  3  4  5  6  7  8*/	
	...
	{27,29,18,34,28,27,27,27,27,menu21     },
	{28,33,18,27,34,28,28,28,28,menu22     },
	...
	{33,33,28,33,33,33,33,33,33,menu221    },
	{34,35,18,28,27,34,34,34,34,menu23     },
	{35,35,34,35,35,35,35,35,35,menu231    },
	...
	{40,43, 5,42,41,40,40,40,40,menu61     },
	{41,44, 5,40,42,41,41,41,41,menu62     },
	{42,45, 5,41,40,42,42,42,42,menu63     },
	{43,43,40,43,43,43,43,43,43,menu611    },
	{44,44,41,44,44,44,44,44,44,menu621    },
	{45,45,42,45,45,45,45,45,45,menu631    },

};


void Task_HMI(uint8_t tick)
{
	uint8_t action = 0;
	action = bsp_GetKey();
...
void menu_main1(uint8_t key)
{
	...
	{
		case 0:
		 	val = intDoseParam.doserate_cps;
			break;
		case 1:
			val = intDoseParam.doserate_uSv_h;
			break;
		case 2:
			val = intDoseParam.doserate_uGy_h;
			break;
		default:
			break;
	}
	Display_doseVal(0,24,val,displayUnit);
...
//	GUI_SetString1616(4,8,"外部剂量率",0);
	...
	{
		val = Chn_Data.value;
		Display_doseVal(0, 24, val, 1);
		GUI_SetString1616(4, 8, "中子剂量率", 0);
	}else{
		GUI_SetString1616(4, 8, "γ剂量率", 0);
		val = extDtDoserate;
		Display_doseVal(0, 24, val, 1);
	}
		...
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				if(index < 3)
				{
					...
					}else if((ps[0] == 1) && (ps[1] == 0) && (ps[2] == 0) && (ps[3] == 0))
					{
						menuIndex = 40;
					}else{
						index = 0;
	...
	switch(key)
	{
		case KEY_ENTER_CLICK:
			if(preMenuIndex == menuIndex)
			{
				...
				menuIndex = menuList[menuIndex].back;
				eepromSetStatus(EEPROM_SET_BASIC_PARAM);
			}else{
...
void menu2(uint8_t key)
{
	...
	GUI_SetString1616(120 + 4, 24 + height * 2, "探测器参数", 0);
}
		...
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				...
				{	
					...
					eepromSetStatus(EEPROM_ADDR_WARN_RANGE);
					menuIndex = menuList[menuIndex].back;
				}
			}
			else
...
void menu311(uint8_t key)
{
	...
	if (back)
	{
		...
		eepromSetStatus(EEPROM_ADDR_INT_DOSE_PARAM);
	}
	else
	{
		GUI_SetString2424(60, 64, "确认清除？", 0);
...
void menu23(uint8_t key)
{
	...
	GUI_SetString1616(120 + 4, 24 + height * 0, "单位设置", 0);
	GUI_SetString1616(120 + 4, 24 + height * 1, "探测器类型", 0);
	GUI_SetString1616(120 + 4, 24 + height * 2, "探测器参数", 1);
}
	...
	switch(key)
	{
		case KEY_ENTER_CLICK:
			if(preMenuIndex == menuIndex)
			{
				displayUnit = unit;
				menuIndex = menuList[menuIndex].back;
				eepromSetStatus(EEPROM_ADDR_DISPLATY_UNIT);
			}else{
				unit = displayUnit;
			}
			break;
...
void menu221(uint8_t key)
{
	...
	switch (key)
	{
		case KEY_ENTER_CLICK:
			...
			{
				detectorType = dt;
				eepromSetStatus(EEPROM_SET_DETECTOR_TYPE);
				menuIndex = menuList[menuIndex].back;
			}else{
				dt = detectorType;
			}
	...
	GUI_SetString1616(4, 24 + height * 0, "FHT 761", dt == 0 ? 1 : 0);
	GUI_SetString1616(4, 24 + height * 1, "FHT 762", dt == 1 ? 1 : 0);
	GUI_SetString1616(4, 24 + height * 2, "FHT 191", dt == 2 ? 1 : 0);
	GUI_SetString1616(4, 24 + height * 3, "FHT 192", dt == 3 ? 1 : 0);

	// GUI_SetString1616(120 + 4, 24 + height * 0, "刻度因子", 0);
	// GUI_SetString1616(120 + 4, 24 + height * 1, "灵敏度", 0);
}
...
void menu231(uint8_t key)
{
		...
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				...
				else
				{
					...
					Chn_Data.fact = (float)(range / 100.00f);
					eepromSetStatus(EEPROM_SET_DETECTOR_PARAM);
					menuIndex = menuList[menuIndex].back;
				}
			}
			else
			{
				index = 0;
				range = (uint32_t)(Chn_Data.fact * 100);
				str[4] = range % 10;
				range /= 10;
				str[3] = range % 10;
				range /= 10;
	...
	GUI_SetString1616(64, 24, "探测器转换系数", 0);

	Display_setDtFactor(60, 48, str, index);
}
		...
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				...
				else
				{
					...
					passWord[2] = ps[2];
					passWord[3] = ps[3];
					eepromSetStatus(EEPROM_SET_PASSWORD);
					menuIndex = menuList[menuIndex].back;
				}
			}
			else
...
void menu61(uint8_t key)
{
	...
	GUI_SetString1616(4, 24 + height * 0, "view count", 1);
	GUI_SetString1616(4, 24 + height * 1, "set factor", 0);
	GUI_SetString1616(4, 24 + height * 2, "set deadtime", 0);
...
void menu62(uint8_t key)
{
	...
	GUI_SetString1616(4, 24 + height * 0, "view count", 0);
	GUI_SetString1616(4, 24 + height * 1, "set factor", 1);
	GUI_SetString1616(4, 24 + height * 2, "set deadtime", 0);
...
void menu63(uint8_t key)
{
	...
	GUI_SetString1616(4, 24 + height * 0, "view count", 0);
	GUI_SetString1616(4, 24 + height * 1, "set factor", 0);
	GUI_SetString1616(4, 24 + height * 2, "set deadtime", 1);
...
void menu611(uint8_t key)
{
	...
	extern uint32_t realCnt ;
	extern uint32_t smoothCnt;
	Display_statusBar();
	...
	Display_CNT(realCnt,smoothCnt);
}

void menu621(uint8_t key)
{
	static uint8_t str[3] = {0, 7, 0};
	static uint8_t index = 0;
	uint32_t val = 0;
	...
	switch (key)
	{
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				...
				{
					val = ((str[0]*10)+str[1])*10+str[2];
					intDoseParam.factor = val * 0.01f;
					eepromSetStatus(EEPROM_SET_INT_DOESE_PARAM);
					menuIndex = menuList[menuIndex].back;
				}
			}
			else
			{
				index = 0;
				val = (uint32_t)(intDoseParam.factor * 100);
				str[2] = val%10;
				val /=10;
				str[1] = val%10;
				val /=10;
	...
	GUI_SetString2424(64, 32, "factor", 0);

	Display_setFactor(60,80,str,index);
...
void menu631(uint8_t key)
{
		...
		case KEY_ENTER_CLICK:
			if (preMenuIndex == menuIndex)
			{
				...
				else
				{
					...
					val = val * 10 +str[6];
					val = val * 10 +str[7];
					intDoseParam.deadtime = val * 0.0000001f;
					eepromSetStatus(EEPROM_SET_INT_DOESE_PARAM);
					menuIndex = menuList[menuIndex].back;
				}
			}
			...
			{
				index = 0;
				val = (uint32_t)(intDoseParam.deadtime * 10000000);
				str[7] = val%10;
				val /=10;
				str[6] = val%10;
				val /=10;
	...
	Display_setDeadtime(4,48,str,index);
