...
void GUI_SetChar5628(uint8_t x,uint8_t y,uint8_t c,uint8_t mode)
{
	uint8_t height = 7;
	uint8_t width = 28;
	...
	for(i=0;i<height;i++)
	{
		for (j = 0; (j < width) && ((x + j) < (LCD_WIDTH - 1)); j++)
		{
			if(mode == 0)
			{
				ramBuf[x+j+(page+i)*LCD_WIDTH] |= ascii5628[(c)*(height*width)+width*i+j];
			}else{
				ramBuf[x+j+(page+i)*LCD_WIDTH] |= ~ascii5628[(c)*(height*width)+width*i+j];
			}
		}
